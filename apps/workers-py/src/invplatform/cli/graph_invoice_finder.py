...
def download_direct_pdf(
    ...
    _ = out_dir  # retained for backwards-compatible function signature
    return pdf_download.download_direct_pdf(
        ...
        referer=referer,
        user_agent=ua,
        include_yes_headers=False,
        fallback_without_referer_on_403=False,
        verbose=False,
    )
...
def is_retryable_reason(reason: str) -> bool:
    return provider_shared.is_retryable_reason(reason)


def load_cached_processed_message_ids(report_path: Optional[str]) -> Set[str]:
    return provider_shared.load_cached_processed_message_ids(report_path)


def decide_pdf_relevance(path: str, trusted_hint: bool = False) -> Tuple[bool, Dict]:
    return pdf_verification.decide_pdf_relevance_graph(
        path,
